...
// Scans all level directories and aggregates file counts and sizes.
func (db *DB) LocalLTXStats() (LTXStats, error) {
	var stats LTXStats

	ltxDir := db.LTXDir()
	levelDirs, err := os.ReadDir(ltxDir)
	...
	for _, levelDir := range levelDirs {
		...
		if _, err := strconv.Atoi(levelDir.Name()); err != nil {
			continue
		}

		levelPath := filepath.Join(ltxDir, levelDir.Name())
